...
class Message:
    ...
    def document(self, chat_id=None, **kwargs):
        return self.bot.document(self.data['chat']['id'] if not chat_id else chat_id, **kwargs)
    ...
    def sticker(self, chat_id=None, **kwargs):
        return self.bot.sticker(self.data['chat']['id'] if not chat_id else chat_id, **kwargs)
    ...
    def edittext(self, text, **kwargs):
        ...
        return self.bot.edittext(text, **kwargs)
        
    def editcaption(self, **kwargs):
        if not 'chat_id' in kwargs:
            kwargs['chat_id'] = self.data['chat_id']
        return self.bot.editcaption(**kwargs)
    
    def editmedia(self, media, **kwargs):
        if not 'chat_id' in kwargs:
            kwargs['chat_id'] = self.data['chat_id']
        return self.bot.editmedia(**kwargs)
...
class Bot:
    ...
    def document(self, chat_id, **kwargs):
        kwargs['chat_id'] = chat_id
    ...
    def sticker(self, chat_id, **kwargs):
        kwargs['chat_id'] = chat_id
    ...
    def register(self, command_, function, types=['message']):
        for type_ in types:
            if type_ in self.commands:
                self.commands[type_]['commands'].append({'run': function, 'match': re.compile(command_)})
    ...
    def more(self, urls):
        async def fetch(url, session):
            async with session.get('https://api.telegram.org/bot{}/{}'.format(url.token, url.method), params=url.params, proxy=self.proxy) as response:
                return json.loads(await response.text())
        async def get():
            async with aiohttp.ClientSession() as session:
